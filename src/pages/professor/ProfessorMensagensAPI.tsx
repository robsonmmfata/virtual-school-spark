...
import { mensagensAPI, alunosAPI } from "@/lib/api.js";
import { useAuth } from "@/hooks/useAuth.js";

const ProfessorMensagensAPI = () => {
  ...
  const carregarDados = async () => {
    try {
      setLoading(true);
      const [conversasData, alunosData] = await Promise.all([
  ...
  const enviarMensagem = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Sua resposta foi entregue ao aluno.",
      });
  ...
  const iniciarConversa = async (alunoId) => {
    try {
      // Verificar se já existe conversa
      const conversaExistente = conversas.find(c => c.outro_usuario_id === alunoId);
      if (conversaExistente) {
        setConversaSelecionada(alunoId);
        return;
      }
      ...
      toast({
        ...
        description: "Sua mensagem foi enviada para o aluno.",
      });
  ...
  const alunoAtual = alunos.find(a => a.id === conversaSelecionada);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Converse com seus alunos e tire suas dúvidas</p>
        </div>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-3">
                      <div className="relative">
                        <div className="w-10 h-10 bg-education-green rounded-full flex items-center justify-center text-white font-semibold">
          ...
          {conversaAtual && alunoAtual ? (
              ...
              <CardHeader className="border-b border-border">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-medium text-foreground">{alunoAtual.nome}</p>
                      <p className="text-sm text-green-600">Matrícula: {alunoAtual.matricula}</p>
                    </div>
                  </div>
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center text-muted-foreground">
                ...
                <p>Escolha um aluno para ver as mensagens ou iniciar uma nova conversa.</p>
              </div>
            </div>
          )}
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-sm text-muted-foreground">Não respondidas</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-education-purple">{alunos.length}</div>
            <div className="text-sm text-muted-foreground">Alunos disponíveis</div>
          </CardContent>
        </Card>
      </div>
    </div>
...
export default ProfessorMensagensAPI;
